...
import { AdminNewsUpdateBodySchema } from '@/lib/api-schemas'
import { jsonFail, jsonOk } from '@/lib/api-helpers'
import { archiveNews, isUpdatesStoreConfigured, listAdminUpdatesSnapshot, updateNews } from '@/lib/updates-store'
...
export async function PUT(request: Request, { params }: RouteProps) {
  ...
  const parsed = AdminNewsUpdateBodySchema.safeParse(rawBody)
  if (!parsed.success) {
  ...
  try {
    ...
    await updateNews(id, parsed.data.news)
    const snapshot = await listAdminUpdatesSnapshot()
    return jsonOk(snapshot.news)
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Haber kaydi guncellenemedi.'
...
export async function DELETE(request: Request, { params }: RouteProps) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Haber arsivlenemedi.'
    return jsonFail(message, 500)
  }
}
